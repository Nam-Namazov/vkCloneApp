...
protocol RowLayoutDelegate: AnyObject {
    func collectionView(
        ...
        photoAtIndexPath indexPath: IndexPath
    ) -> CGSize
}
...
final class RowLayout: UICollectionViewLayout {
    ...
    override func prepare() {
        ...
        for item in 0 ..< collectionView.numberOfItems(inSection: 0) {
            ...
            let photoSize = delegate.collectionView(collectionView,
        ...
        var xOffset = [CGFloat](repeating: 0,
                                count: RowLayout.numbersOfRows)
        var row = 0
        for item in 0 ..< collectionView.numberOfItems(inSection: 0) {
            let indexPath = IndexPath(item: item, section: 0)
    ...
    static func rowHeightCounter(superviewWidth: CGFloat,
                                 photosArray: [CGSize]) -> CGFloat? {
